...
export default function Dashboard() {
  ...
  // Track the active CSPR.click account (poll — works across connect/switch/disconnect).
  useEffect(() => {
    if (!click) return;
    let alive = true;
    const sync = () =>
      click
        .getActivePublicKey?.()
        .then((pk: string | undefined) => alive && setAccount(pk ?? null))
  ...
  const connect = () => {
    if (!click) return;
    connected ? click.signOut?.() : click.signIn?.();
  };
